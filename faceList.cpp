...
class Face
{
	...
	{
		/*if(x<0.5)
		return (0.32+0.08*(x+1)*(x+1));
		else
		return ((4.0/3.5)-(1.0/3.5)*(x+1)*(x+1));
		*/
		//return 0.6*(2.2733+1.272*x-1.64*x*x-1.4753*x*x*x);
		if(x==1)
			return 0;
		if(x>=-1 && x<=-0.8)
			return 0.3;
	...
	float fderv(float x)
	{
		...
		return -(2.0/3.5)*(x+1);
		*/
		//return 0.6*(1.272-2*1.64*x-3*1.4753*x*x);
		if(x>-1 && x<=-0.8)
			return 0;
		else if(x>-0.8 && x<0.0)
